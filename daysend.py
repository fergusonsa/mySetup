...
NOTE_FILE_LINE_PATTERN = re.compile("^(?P<year>\d{4})-(?P<month>\d{2})-(?P<day>\d{2}) (?P<hours>\d{2}):(?P<minutes>\d{2})(:(?P<seconds>\d{2}))?( (?P<ampm>[AP]M))?\s*(?P<rest>.*)$")
'''2018-12-17 15:57    NEW_EMPLOYEE_SETUP    Jira access http://jira.ic.gc.ca/login.jsp'''
...
def parse_line(line):
    parts = NOTE_FILE_LINE_PATTERN.match(line)
    if not parts:
        parts = NOTE_FILE_LINE_PATTERN_2.match(line)
    if parts:
        # print(parts.groupdict())
        hours = int(parts.groupdict()['hours'])
        if parts.groupdict()['ampm'] == 'PM' and hours < 12:
            hours += 12
...
def get_notes_history(end_date=None, start_date=None):
    ...
    dir_path = os.path.abspath(os.path.join('/', 'cygdrive', 'p', 'notes'))
...
def print_notes_times(end_of_week_date=None, hours_data=None):
    ...
    for key in sorted(hours_data.keys()):
        val = hours_data[key]
